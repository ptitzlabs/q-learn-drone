...
void rate_controller::parm_init(
    ...
    _parm.drone = *drone;  // make a private copy of drone model

    // Controller I/O parameters
    _parm.n_input_states = n_input_states;
    _parm.n_goal_states = n_goal_states;

    _parm.
    _parm.id_control_input = control_input_id;
    _parm.id_control_levels
    ...
    _parm.input_parm_id = new int[_parm.n_input_parms];

    for (int i = 0; i < n_input_states; i++) _parm.input_parm_id[i] = parm_id[i];
    for (int i = 0; i < n_goal_states; i++)
        _parm.input_parm_id[i + n_input_states] = goal_state_id[i];
    ...
    _parm.net.parm_init(n_input_states + n_goal_states,  // both the drone state and
