...
drone_dynamics::drone_dynamics() {
    ...
    _parm = new double[_n_parms];       // parameter storage
    _init_parm = new double[_n_parms];  // parameter default

    for (int i = 0; i < 6; i++) {
        _x[i] = &_parm[i];         // x assigned index 0:5
        _xd[i] = &_parm[i + 6];    // xd assigned index 6:11
        _xdd[i] = &_parm[i + 12];  // xdd assigned index 12:17
    }

    for (int i = 0; i < _n_aux; i++) {
        _aux[i] = &_parm[i + 18];  // auxillary parameters are appended after
                                   // state parameters
    }
    for (int i = 0; i < _n_parms; i++) {
...
double drone_dynamics::get_parm(int id) { return _parm[id]; }
...
void drone_dynamics::get_state(double *state, int *id, int n_states) {
    ...
    for (int i = 0; i < n_states; i++)
        std::cout << "id: " << id[i] << " val: " << _parm[id[i]] << "\t";
    // state[i] = _parm[id[i]];
}
