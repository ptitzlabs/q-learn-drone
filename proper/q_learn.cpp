...
bool q_learn::goal_reached(int n) {
    if (_m->get_state(n) > _curr_goal[n]) return true;
    else return false;
}
...
void q_learn::report(){
    ...
    for (int i = 0; i<_n_action_levels ; i++) std::cout<<_action_levels[i]<<"  ";
    std::cout<<"\nNumber of goal states: "<<_n_goal_states<<"\nCurrent goal state:";
    for (int i = 0; i<_n_goal_states ; i++) std::cout<<_curr_goal[_goal_state_index[i]]<<"  ";
